...
const props = defineProps(['script','genres'])

const showLeadRoles = ref(true)
...
const leadRoles = reactive({values: JSON.parse(props.script.script_lead_roles)})
...
const copyright = reactive({values: JSON.parse(props.script.copyright)})

const genresIds = props.script.genres.map((genre) => {
  return genre.id
})

const subGenreIds = props.script.sub_genres.map((genre) => {
  return genre.id
})

const form = useForm({
  "title": props.script.script_title,
  "posterImage": null,
  "logline": props.script.script_logline,
  "synopsis": props.script.script_synopsis,
  "motivation": props.script.motivation,
  "relevance": props.script.relevance,
  "origin": props.script.story_origin,
  "scriptDocument": null,
  "mainGenre": genresIds,
  "subGenre": subGenreIds,
  "movieFormat": props.script.movie_format,
  "castSize": props.script.script_cast_size,
  "location": props.script.script_no_locations,
  "leadRoles": leadRoles.values,
  "copyright": copyright.values,
  "movie_format": props.script.movie_format,
  "target_audience": props.script.target_audience,
  "motivation": props.script.motivation,
  "relevance": props.script.relevance,
  "story_origin": props.script.story_origin,
})
...
function submitForm(){
  form.put(route('scriptwriter.script.edit.put', props.script.id))
}
...
<template>
  <ScriptWriterDashboardLayout>
    ...
    <div class="form-container">
        ...
        <h1 v-else class="text-center text-2xl uppercase font-bold">Edit Script</h1>
